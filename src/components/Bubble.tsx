...
export function Home() {
  ...
  useEffect(() => {
    ...
    console.log("The greatest radius is: " + maxRadius);

    // Add the circles to the SVG element, assigning a color to each top-level node
    svg
      .selectAll("circle")
      .data(nodes.slice(data.children.length + 1)) // skip the first node to remove the outer circle - count(data) or data.children.length + 1
      .join("circle")
      .attr("r", (d) => {
